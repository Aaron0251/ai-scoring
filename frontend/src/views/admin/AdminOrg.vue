<template>
  <AppLayout>
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
      <h2 class="page-title" style="margin:0">組織架構管理</h2>
      <div style="display:flex;gap:8px">
        <el-button @click="downloadOrgTemplate" icon="Download">下載人員範本</el-button>
        <el-upload
          :auto-upload="false"
          :show-file-list="false"
          accept=".xlsx,.xls"
          :on-change="handleOrgImport"
        >
          <el-button type="primary" :loading="orgImporting" icon="Upload">匯入組織人員</el-button>
        </el-upload>
      </div>
    </div>
    ...
    <el-tabs>
      ...
      <el-tab-pane label="本部">
        <div style="display:flex;gap:8px;margin-bottom:8px">
          ...
          <el-button type="danger" :disabled="selectedDivs.length === 0" @click="deleteSelectedDivs">刪除選取（{{ selectedDivs.length }}）</el-button>
        </div>
        <el-table :data="divisions" stripe size="small" @selection-change="selectedDivs = $event">
      ...
      <el-tab-pane label="部門">
        <div style="display:flex;gap:8px;margin-bottom:8px">
          ...
          <el-button type="danger" :disabled="selectedDepts.length === 0" @click="deleteSelectedDepts">刪除選取（{{ selectedDepts.length }}）</el-button>
        </div>
        <el-table :data="departments" stripe size="small" @selection-change="selectedDepts = $event">
          <el-table-column type="selection" width="45" />
      ...
      <el-tab-pane label="課級">
        <div style="display:flex;gap:8px;margin-bottom:8px">
          ...
          <el-button type="danger" :disabled="selectedSections.length === 0" @click="deleteSelectedSections">刪除選取（{{ selectedSections.length }}）</el-button>
        </div>
        <el-table :data="sections" stripe size="small" @selection-change="selectedSections = $event">
          <el-table-column type="selection" width="45" />
...
function personsOf(type, id) {
  if (type === 'division')   return allPersons.value.filter(p => p.divisionId === id && !p.departmentId && !p.sectionId)
  if (type === 'department') return allPersons.value.filter(p => p.departmentId === id && !p.sectionId)
...
async function downloadOrgTemplate() {
  const token = sessionStorage.getItem('token')
  const res = await axios.get('/api/import/org-template', {
    headers: { Authorization: `Bearer ${token}` },
    responseType: 'blob',
  })
  const url = URL.createObjectURL(res.data)
  const a = document.createElement('a'); a.href = url; a.download = 'org_persons_template.xlsx'; a.click()
  URL.revokeObjectURL(url)
}

async function handleOrgImport(file) {
  orgImporting.value = true
  const formData = new FormData()
  formData.append('file', file.raw)
  const token = sessionStorage.getItem('token')
  try {
    const res = await axios.post('/api/import/org-persons', formData, {
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'multipart/form-data' },
    })
    const { successRows, failedRows } = res.data
    ElMessage.success(`匯入完成：成功 ${successRows} 筆，失敗 ${failedRows} 筆`)
    await load()
  } catch (e) {
    ElMessage.error(e.response?.data?.error || '匯入失敗')
  } finally {
    orgImporting.value = false
  }
}
...
// ── 批次刪除 ─────────────────────────────────────────────────
async function deleteSelectedDivs() {
  try {
    await ElMessageBox.confirm(`確定刪除選取的 ${selectedDivs.value.length} 筆本部？`, '確認', { type: 'warning' })
  } catch { return }
  try {
    await Promise.all(selectedDivs.value.map(r => divisionsApi.remove(r.id)))
    ElMessage.success('已刪除')
    load()
  } catch (e) { ElMessage.error(e.response?.data?.error || '刪除失敗') }
}
async function deleteSelectedDepts() {
  try {
    await ElMessageBox.confirm(`確定刪除選取的 ${selectedDepts.value.length} 筆部門？`, '確認', { type: 'warning' })
  } catch { return }
  try {
    await Promise.all(selectedDepts.value.map(r => departmentsApi.remove(r.id)))
    ElMessage.success('已刪除')
    load()
  } catch (e) { ElMessage.error(e.response?.data?.error || '刪除失敗') }
}
async function deleteSelectedSections() {
  try {
    await ElMessageBox.confirm(`確定刪除選取的 ${selectedSections.value.length} 筆課級？`, '確認', { type: 'warning' })
  } catch { return }
  try {
    await Promise.all(selectedSections.value.map(r => sectionsApi.remove(r.id)))
    ElMessage.success('已刪除')
    load()
  } catch (e) { ElMessage.error(e.response?.data?.error || '刪除失敗') }
}
